...
def main():
    ...
    family_set = set(family_df[family_df["SAMPLE"].isin(multivcf_list)]["Familia"]) # Get all the family ID of all the samples of the database
    family_set.remove("-") # Do not take into account samples without family ID
    muestras_out = list(family_df[family_df["Familia"].isin(family_set)]["SAMPLE"]) # Get all family members of those families
    ...
    sub_df = family_df[family_df["SAMPLE"].isin(muestras_in)][["SAMPLE","Familia"]]
    fam_muestra_dict = sub_df.groupby('Familia')['SAMPLE'].apply(lambda x: x.tolist()) # Make a dictiionary where keys are family IDs and values are list of samples in that family
