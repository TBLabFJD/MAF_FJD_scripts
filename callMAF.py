...
def main(args):
    ...
    cut_dict = {'Categoria': hl.agg.filter(hl.is_defined(mt.Categoria), hl.agg.counter(mt.Categoria))}
    ...
    sample_group_filters.extend([
            ({'DS': Categoria}, mt.Categoria == Categoria) for Categoria in cut_data.Categoria
        ] +  [
            ({'P': Categoria}, mt.Categoria != Categoria) for Categoria in cut_data.Categoria
        ])
